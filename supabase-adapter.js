...
GAS.getTodaySalesData = function() {
  ...
  return sbGet('order_items',
    'select=order_code,label,qty,subtotal,status&ts=gte.' + encodeURIComponent(gte) + '&ts=lte.' + encodeURIComponent(lte) + '&limit=10000'
  ).then(function(rows) {
    var orderIds = {};
    var itemMap  = {};
    var total    = 0;
...
GAS.getLatestOrderDate = function() {
  return sbGet('order_items', 'select=ts&order=id.desc&limit=1').then(function(rows) {
